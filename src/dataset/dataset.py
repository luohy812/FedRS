...
def scan_path(d_name, d_path):
    ...
    if d_name == 'LiTS':
        for f in os.listdir(d_path):
            if f.startswith('volume-') and f.endswith('.nii'):
                ...
                if os.path.isfile('{}/segmentation-{}.nii'.format(d_path, id)):
                    case_name = 'volume-{}'.format(id)
                    image_name = '{}/volume-{}.nii'.format(d_path, id)
                    label_name = '{}/segmentation-{}.nii'.format(d_path, id)
                    entries.append([d_name, case_name, image_name, label_name])
    elif d_name == 'KiTS':
    ...
    #                 entries.append([d_name, case_name, image_name, label_name])
        ...
        for f in os.listdir('{}/imagesTr_downsampled'.format(d_path)):
            if f.startswith('spleen_'):
                id = int(f.split('.nii.gz')[0].split('spleen_')[1])
                if os.path.isfile('{}/labelsTr_downsampled/{}'.format(d_path, f)):
                    case_name = 'spleen_{}'.format(id)
                    image_name = '{}/imagesTr_downsampled/{}'.format(d_path, f)
                    label_name = '{}/labelsTr_downsampled/{}'.format(d_path, f)
                    entries.append([d_name, case_name, image_name, label_name])
        ...
        for f in os.listdir('{}/imagesTr_downsampled'.format(d_path)):
            if f.startswith('lung_'):
                id = int(f.split('.nii.gz')[0].split('lung_')[1])
                if os.path.isfile('{}/labelsTr_downsampled/{}'.format(d_path, f)):
                    case_name = 'lung_{0:03d}'.format(id)
                    image_name = '{}/imagesTr_downsampled/{}'.format(d_path, f)
                    label_name = '{}/labelsTr_downsampled/{}'.format(d_path, f)
                    entries.append([d_name, case_name, image_name, label_name])
    elif d_name == 'colon':
        for f in os.listdir('{}/imagesTr_downsampled'.format(d_path)):
            if f.startswith('colon_'):
                id = int(f.split('.nii.gz')[0].split('colon_')[1])
                if os.path.isfile('{}/labelsTr_downsampled/{}'.format(d_path, f)):
                    case_name = 'colon_{0:03d}'.format(id)
                    image_name = '{}/imagesTr_downsampled/{}'.format(d_path, f)
                    label_name = '{}/labelsTr_downsampled/{}'.format(d_path, f)
                    entries.append([d_name, case_name, image_name, label_name])
    elif d_name == 'hepaticvessel':
        for f in os.listdir('{}/imagesTr_downsampled'.format(d_path)):
            if f.startswith('hepaticvessel_'):
                id = int(f.split('.nii.gz')[0].split('hepaticvessel_')[1])
                if os.path.isfile('{}/labelsTr_downsampled/{}'.format(d_path, f)):
                    case_name = 'hepaticvessel_{0:03d}'.format(id)
                    image_name = '{}/imagesTr_downsampled/{}'.format(d_path, f)
                    label_name = '{}/labelsTr_downsampled/{}'.format(d_path, f)
                    entries.append([d_name, case_name, image_name, label_name])
    return entries
...
def create_folds(dataset_name, dataset_path, fold_name, fraction, exclude_case):
    fold_file_name = '{0:s}/data_split-{1:s}.txt'.format(sys.path[0], fold_name) #sys.path[0]是当前运行代码下所在的目录
    folds = {}
...
class ClientDataset(data.Dataset):
    def __init__(self, ids, rs_size, rs_spacing, rs_intensity, label_map, cls_num, aug_data, full_sampling, enforce_fg,
                 fixed_sample):
        ...
        for record_id, [d_name, casename, image_fn, label_fn] in enumerate(ids):
            ...
            if not self.full_sampling:  # randomly sample one patch from each image during training/validation
                ...
                if not os.path.exists(buffer_fn_z):
                    ...
                    lb_reader.SetFileName(label_fn) #处理标签信息
                    ...
                    nonzero_pixels_y = {}
                    nonzero_pixels_x = {}
                    mapped_array = np.zeros_like(lb_array) #新的label
                    mapped_array_values = []
                    for key in lmap: #遍历的是lmap中的key 遍历lmap的key（冒号前）字典的嵌套
                        mapped_array[lb_array == key] = lmap[key] #{1：1，2：1}，map[]=Lamp[1]
                    ...
                    for key in mapped_array_values:
                        ...
                        tmp_array[mapped_array == key] = 1  # 统计非0区
                        if np.sum(tmp_array) == 0: # 如果=0说明全0
                ...
                if not self.fixed_sample:
                    ...
                    for key in nonzero_pixels_z:
                        ...
                        item_record['label_filename'] = label_fn
                        item_record['nonzero_pixels'] = [nonzero_pixels_x, nonzero_pixels_y, nonzero_pixels_z]
                        item_record['median_coord'] = [np.median(nonzero_pixels_x[key]),
                                                       np.median(nonzero_pixels_y[key]),
                                                       np.median(nonzero_pixels_z[key])]
                        item_record['nonzero_pix_sample_weights'] = nonzero_pix_sample_weights
                        item_record['patch_pos'] = np.array([0, 0, 0], dtype=int)
                        item_record['patch_stride'] = np.array([0, 0, 0], dtype=int)
                        item_record['patch_grid_size'] = np.array([1, 1, 1], dtype=int)
                ...
                image_phy_size = np.array([0, 0, 0], dtype=float)  # 图像真实尺寸
                patch_phy_size = np.array([0, 0, 0], dtype=float)  # patch尺寸
                patch_stride_size = np.array([0, 0, 0], dtype=int)
                patch_grid_origin = np.array([0, 0, 0], dtype=float)
                for i in range(3):
                    image_phy_size[i] = size[i] * spacing[i] #真实图片尺寸
                    patch_phy_size[i] = rs_size[i] * rs_spacing[i] #我希望的图片尺寸
                    if i == 0:
                        patch_grid_size[i] = 2
                        ...
                        if patch_phy_size[i] > 0.5 * image_phy_size[i]:
                            patch_stride_size[i] = int(
                                (origin[i] + image_phy_size[i] - patch_phy_size[i] - patch_grid_origin[i]) / rs_spacing[
                                    i])
                        else:
                            patch_stride_size[i] = int(
                                (origin[i] + 0.5 * image_phy_size[i] - patch_grid_origin[i]) / rs_spacing[i])
                    elif i == 1:
                        ...
                        while patch_phy_size[i] + (patch_grid_size[i] - 1) * patch_stride_size[i] * rs_spacing[i] < \
                                image_phy_size[i]:
                            patch_grid_size[i] += 1
                        patch_grid_origin[i] = origin[i] + 0.5 * image_phy_size[i] - 0.5 * (
                                    patch_phy_size[i] + (patch_grid_size[i] - 1) * patch_stride_size[i] * rs_spacing[i])

                total_patch_num = patch_grid_size[0] * patch_grid_size[1] * patch_grid_size[2]
                # print(origin[0])
                # print(patch_stride_size[0])
    ...
    def get_random_patch_origin(self, item_record, origin, size, spacing, fg_prob):
        enforce_fg_sample = (self.loaded_sample_num % 2 == 1 or self.enforce_fg) #如果 loaded_sample_num 是奇数或 enforce_fg 为 True，则 enforce_fg_sample 为 True
        patch_origin = np.array([0, 0, 0], dtype=float)#建立一个初始patch的起点
        is_fg_patch = False #给这个值设为False
        if (enforce_fg_sample or (np.random.uniform(0.0, 1.0) < fg_prob)) and len(item_record['nonzero_pixels'][2]) > 0:
            # print('前景选择')
            #enforce_fg_sample 为 True或者随机生成的数小于 fg_prob（即根据概率选择前景样本） z轴大于0
            if self.fixed_sample:
                ...
                selected_label = np.random.choice(list(item_record['nonzero_pixels'][2].keys()),
                                                  p=item_record['nonzero_pix_sample_weights'])
                ...
                # print(np.random.choice(len(selected_label_pos_x)))
            ...
            # print(selected_pixel_pos)
            for i in range(3):
                patch_origin[i] = origin[i] + selected_pixel_pos[i] * spacing[i] - (self.rs_size[i] // 2) * \
                                  self.rs_spacing[i]

                if size[i] * spacing[i] < self.rs_size[i] * self.rs_spacing[i]:
                    patch_origin[i] = origin[i] + size[i] * spacing[i] * 0.5 - self.rs_size[i] * self.rs_spacing[
                        i] * 0.5
                else:
            ...
            for i in range(3):
                if size[i] * spacing[i] < self.rs_size[i] * self.rs_spacing[i]:
                    ...
                    if int((size[i] * spacing[i] - self.rs_size[i] * self.rs_spacing[i]) / spacing[i]) > 0:
                        patch_origin[i] = origin[i] + spacing[i] * np.random.choice(
                            int((size[i] * spacing[i] - self.rs_size[i] * self.rs_spacing[i]) / spacing[i]))
                    else:
                        patch_origin[i] = origin[i]
        # print(patch_origin)
    ...
    def __getitem__(self, index): #
        ...
        if item_record['image_filename'] not in self.im_cache:
            ...
            src_image = array_2_image(image_cache['array'], image_cache['spacing'], image_cache['origin'],
                                      self.ImageType)

        is_fg_patch = False
        if not self.full_sampling:
            patch_origin, is_fg_patch = self.get_random_patch_origin(item_record, image_cache['origin'],
                                                                     image_cache['size'], image_cache['spacing'], 0.333)
        else:
        ...
        image['array'] = normalize(image['array'], min=self.rs_intensity[0], max=self.rs_intensity[1],
                                   mean=self.rs_intensity[2], std=self.rs_intensity[3], aug=False)
        ...
        if os.path.exists(item_record['label_filename']):
            if item_record['label_filename'] not in self.lb_cache:
                ...
                src_label = array_2_image(label_cache['array'], label_cache['spacing'], label_cache['origin'],
                                          self.LabelType)
            ...
            # print(torch.unique(output['label']))
            output['label_exist'] = label_exist

        #     print( output['label_fname'])
        #     print('tensor',torch.unique(output['label']))
        return output
...
#                 try:
#                     # 尝试使用 nibabel 加载 nii 文件
#                     img = nib.load(filepath)
#                     # 可以选择检查图像数据的某些属性
#                     print(f"File {filepath} loaded successfully. Shape: {img.shape}")
#                 except Exception as e:
#                     # 捕获并打印异常，表示文件可能损坏或读取失败
#                     print(f"Failed to load {filepath}. Error: {e}")
#     print(c)
# # 调用函数，传入你的 NIfTI 文件所在的目录
# check_nii_files('/public1/lhy/data/FedMENU/LiTS/LiTS_downsampled')